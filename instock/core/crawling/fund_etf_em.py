...
def fund_etf_spot_em() -> pd.DataFrame:
    ...
    try:
        ...
        logging.error(f"获取东方财富-ETF实时行情数据失败: {e}")
        return pd.DataFrame()
    except Exception as e:
        logging.error(f"处理东方财富-ETF实时行情数据失败: {e}")
        return pd.DataFrame()
...
def _fund_etf_code_id_map_em() -> dict:
    ...
    try:
        ...
        logging.error(f"获取东方财富-ETF代码和市场标识映射数据失败: {e}")
        return {}
    except Exception as e:
        logging.error(f"处理东方财富-ETF代码和市场标识映射数据失败: {e}")
        return {}
...
def fund_etf_hist_em(
    ...
    try:
        ...
        if not code_id_dict:
            logging.error("ETF代码和市场标识映射数据为空")
            return pd.DataFrame()
        if symbol not in code_id_dict:
            logging.error(f"ETF代码 {symbol} 不在映射数据中")
            return pd.DataFrame()
        ...
        logging.error(f"获取东方财富-ETF历史行情数据失败: {e}")
        return pd.DataFrame()
    except Exception as e:
        logging.error(f"处理东方财富-ETF历史行情数据失败: {e}")
        return pd.DataFrame()
...
def fund_etf_hist_min_em(
    ...
    try:
        ...
        logging.error(f"获取东方财富-ETF分时行情数据失败: {e}")
        return pd.DataFrame()
    except Exception as e:
        logging.error(f"处理东方财富-ETF分时行情数据失败: {e}")
        return pd.DataFrame()
