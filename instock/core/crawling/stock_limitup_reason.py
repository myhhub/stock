...
def stock_limitup_reason(date: str = "2025-02-27") -> pd.DataFrame:
    ...
    try:
        ...
        r = requests.get(url, proxies = proxys().get_proxies(), headers=headers, timeout=10)
        ...
        logging.error(f"获取同花顺涨停原因数据失败: {e}")
        return pd.DataFrame()
    except Exception as e:
        logging.error(f"处理同花顺涨停原因数据失败: {e}")
        return pd.DataFrame()
...
def stock_limitup_detail(row):
    ...
    try:
        ...
        logging.error(f"获取同花顺涨停详因数据失败: {e}")
        return ""
    except Exception as e:
        logging.error(f"处理同花顺涨停详因数据失败: {e}")
        return ""
