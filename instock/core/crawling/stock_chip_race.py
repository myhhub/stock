...
def stock_chip_race_open(date: str = "") -> pd.DataFrame:
    ...
    try:
        ...
        r = requests.post(url, proxies = proxys().get_proxies(), json=params, headers=headers, timeout=10)
        ...
        logging.error(f"获取通达信竞价抢筹早盘抢筹数据失败: {e}")
        return pd.DataFrame()
    except Exception as e:
        logging.error(f"处理通达信竞价抢筹早盘抢筹数据失败: {e}")
        return pd.DataFrame()
...
def stock_chip_race_end(date: str = "") -> pd.DataFrame:
    ...
    try:
        ...
        logging.error(f"获取通达信竞价抢筹尾盘抢筹数据失败: {e}")
        return pd.DataFrame()
    except Exception as e:
        logging.error(f"处理通达信竞价抢筹尾盘抢筹数据失败: {e}")
        return pd.DataFrame()
