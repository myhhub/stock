...
def get_indicators(data, end_date=None, threshold=120, calc_threshold=None):
    try:
        ...
        data.loc[:, 'm_price_sf1'] = data['m_price'].shift(1).fillna(0.0)
        data.loc[:, 'h_m'] = data['high'] - data[['m_price_sf1', 'high']].min(axis=1)
        data.loc[:, 'm_l'] = data['m_price_sf1'] - data[['m_price_sf1', 'low']].min(axis=1)
        data.loc[:, 'h_m_sum'] = data['h_m'].rolling(window=26).sum().fillna(0.0)
        data.loc[:, 'm_l_sum'] = data['m_l'].rolling(window=26).sum().fillna(0.0)
        data.loc[:, 'cr'] = (data['h_m_sum'] / data['m_l_sum']).fillna(0.0) * 100
        ...
        data.loc[:, 'avs'] = data['av'].rolling(min_periods=1, window=26, center=False).sum().fillna(0.0)
        data.loc[:, 'bv'] = data['volume'].where(data['p_change'] < 0, 0, inplace=False)
        data.loc[:, 'bvs'] = data['bv'].rolling(min_periods=1, window=26, center=False).sum().fillna(0.0)
        data.loc[:, 'cv'] = data['volume'].where(data['p_change'] == 0, 0, inplace=False)
        data.loc[:, 'cvs'] = data['cv'].rolling(min_periods=1, window=26, center=False).sum().fillna(0.0)
        data.loc[:, 'vr'] = (data['avs'] + data['cvs'] / 2) / (data['bvs'] + data['cvs'] / 2) * 100
        data.loc[:, 'vr_6_sma'] = tl.MA(data['vr'], timeperiod=6).fillna(0.0).fillna(0.0)
        # atr
        data.loc[:, 'prev_close'] = data['close'].shift(1).fillna(0.0)
        ...
        # stockstats计算公式
        data.loc[:, 'high_delta'] = data['high'].diff(1).fillna(0.0)
        data.loc[:, 'high_m'] = (data['high_delta'] + data['high_delta'].abs()) / 2
        data.loc[:, 'low_delta'] = -data['low'].diff(1).fillna(0.0)
        ...
        data.loc[:, 'pdi'] = (data['pdm'] / data['atr']).fillna(0.0) * 100
        data.loc[:, 'mdm'] = tl.EMA(data['low_m'].where(data['low_m'] > data['high_m'], 0), timeperiod=14).fillna(0.0)
        data.loc[:, 'mdi'] = (data['mdm'] / data['atr']).fillna(0.0) * 100
        data.loc[:, 'dx'] = ((data['pdi'] - data['mdi']).abs() / (data['pdi'] + data['mdi'])).fillna(0.0) * 100
        data.loc[:, 'adx'] = tl.EMA(data['dx'], 6).fillna(0.0)
        data.loc[:, 'adxr'] = tl.EMA(data['adx'], 6).fillna(0.0)
        # wr
        data.loc[:, 'h_6'] = data['high'].rolling(window=6).max().fillna(0.0)
        data.loc[:, 'l_6'] = data['low'].rolling(window=6).min().fillna(0.0)
        data.loc[:, 'wr_6'] = ((data['h_6'] - data['close']) / (data['h_6'] - data['l_6'])).fillna(0.0) * 100
        data.loc[:, 'h_10'] = data['high'].rolling(window=10).max().fillna(0.0)
        data.loc[:, 'l_10'] = data['low'].rolling(window=10).min().fillna(0.0)
        data.loc[:, 'wr_10'] = ((data['h_10'] - data['close']) / (data['h_10'] - data['l_10'])).fillna(0.0) * 100
        ...
        data.loc[:, 'vol_14'] = data['volume'].rolling(window=14).sum().fillna(0.0)
        data.loc[:, 'vwma'] = (data['tpv_14'] / data['vol_14']).fillna(0.0)
        # ppo
        ...
        data.loc[:, 'ppo'] = ((data['ppo_short'] - data['ppo_long']) / data['ppo_long']).fillna(0.0) * 100
        data.loc[:, 'ppos'] = tl.EMA(data['ppo'], timeperiod=9).fillna(0.0)
        data.loc[:, 'ppoh'] = data['ppo'] - data['ppos']
        # stochrsi
        data.loc[:, 'rsi_min'] = data['rsi'].rolling(window=14).min().fillna(0.0)
        data.loc[:, 'rsi_max'] = data['rsi'].rolling(window=14).max().fillna(0.0)
        data.loc[:, 'stochrsi'] = ((data['rsi'] - data['rsi_min']) / (data['rsi_max'] - data['rsi_min'])).fillna(0.0) * 100
        # wt
        ...
        data.loc[:, 'esa_ci'] = ((data['m_price'] - data['esa']) / (0.015 * data['esa_d'])).fillna(0.0)
        ...
        data.loc[:, 'price_up_sum'] = data['price_up'].rolling(window=12).sum().fillna(0.0)
        data.loc[:, 'psy'] = (data['price_up_sum'] / 12.0) * 100
        # BRAR
        data.loc[:, 'h_o'] = data['high'] - data['open']
        data.loc[:, 'o_l'] = data['open'] - data['low']
        data.loc[:, 'h_o_sum'] = data['h_o'].rolling(window=26).sum().fillna(0.0)
        data.loc[:, 'o_l_sum'] = data['o_l'].rolling(window=26).sum().fillna(0.0)
        data.loc[:, 'ar'] = (data['h_o_sum'] / data['o_l_sum']).fillna(0.0) * 100
        data.loc[:, 'h_c'] = data['high'] - data['close']
        data.loc[:, 'c_l'] = data['close'] - data['low']
        data.loc[:, 'h_c_sum'] = data['h_c'].rolling(window=26).sum().fillna(0.0)
        data.loc[:, 'c_l_sum'] = data['c_l'].rolling(window=26).sum().fillna(0.0)
        data.loc[:, 'br'] = (data['h_c_sum'] / data['c_l_sum']).fillna(0.0) * 100
        # EMV
        data.loc[:, 'prev_high'] = data['high'].shift(1).fillna(0.0)
        data.loc[:, 'prev_low'] = data['low'].shift(1).fillna(0.0)
        ...
        data.loc[:, 'bias_12'] = ((data['close'] - data['ma12']) / data['ma12']).fillna(0.0) * 100
        data.loc[:, 'bias_24'] = ((data['close'] - data['ma24']) / data['ma24']).fillna(0.0) * 100
        # DPO
        ...
        # VHF
        data.loc[:, 'hcp_lcp'] = tl.MAX(data['close'], timeperiod=28) - tl.MIN(data['close'], timeperiod=28).fillna(0.0)
        data.loc[:, 'vhf'] = data['hcp_lcp'].div(tl.MA((data['close'] - data['prev_close']).abs(), timeperiod=28) * 28).fillna(0.0)
        # RVI
        data.loc[:, 'rvi_x'] = data['close'] - data['open'] + 2 * (data['prev_close'] - data['open'].shift(1).fillna(0.0)) + 2 * (
                data['close'].shift(2).fillna(0.0) - data['open'].shift(2).fillna(0.0)) * (data['close'].shift(3).fillna(0.0) - data['open'].shift(3).fillna(0.0)) / 6
        data.loc[:, 'rvi_y'] = data['high'] - data['low'] + 2 * (data['prev_high'] - data['prev_low']) + 2 * (
                data['high'].shift(2).fillna(0.0) - data['low'].shift(2).fillna(0.0)) * (data['high'].shift(3).fillna(0.0) - data['low'].shift(3).fillna(0.0)) / 6
        data.loc[:, 'rvi'] = tl.MA(data['rvi_x'], timeperiod=10) * 10 / tl.MA(data['rvi_y'], timeperiod=10).fillna(0.0) * 10
        # FI
        ...
        logging.debug("{}处理异常：{}代码{}".format('talib_indicator.get_indicators', data['code'], e))
    return None
