...
def get_indicators(data, end_date=None, threshold=120, calc_threshold=None):
    ...
    try:
        ...
        data.loc[:, 'vr'] = ((data['avs'] + data['cvs'] / 2) / (data['bvs'] + data['cvs'] / 2)).replace(np.nan,
                                                                                                        0).replace(
            np.inf, 0) * 100
        ...
        data.loc[:, 'atr'] = tl.ATR(data['high'], data['low'], data['close'], timeperiod=14)
        # DMI
        ...
        data.loc[:, 'pdm'] = tl.EMA(data['high_m'].where(data['high_m'] > data['low_m'], 0), timeperiod=14)
        data.loc[:, 'pdi'] = data['pdm'] / data['atr'] * 100
        data.loc[:, 'mdm'] = tl.EMA(data['low_m'].where(data['low_m'] > data['high_m'], 0), timeperiod=14)
        data.loc[:, 'mdi'] = data['mdm'] / data['atr'] * 100
        data.loc[:, 'dx'] = (data['pdi'] - data['mdi']).abs() / (data['pdi'] + data['mdi']) * 100
        data.loc[:, 'adx'] = tl.EMA(data['dx'], 6)
        data.loc[:, 'adxr'] = tl.EMA(data['adx'], 6)
        # wr
        ...
        data.loc[:, 'dma'] = (data['ma10'] - data['ma50']).fillna(0.0)
        ...
        # DPO
        data.loc[:, 'dpo'] = data['close'] - (tl.MA(data['close'], timeperiod=11)).shift(1)
        data.loc[:, 'madpo'] = tl.MA(data['dpo'], timeperiod=6)
        # VHF
        data.loc[:, 'hcp_lcp'] = tl.MAX(data['close'], timeperiod=28) - tl.MIN(data['close'], timeperiod=28)
        data.loc[:, 'vhf'] = data['hcp_lcp'].div(tl.MA((data['close'] - data['prev_close']).abs(), timeperiod=28) * 28)
        # RVI
        ...
        data.loc[:, 'rvi'] = tl.MA(data['rvi_x'], timeperiod=10) * 10 / tl.MA(data['rvi_y'], timeperiod=10) * 10
        # FI
        data.loc[:, 'fi'] = data['close'].diff(1) * data['volume']
        data.loc[:, 'force_2'] = tl.EMA(data['fi'], timeperiod=2)
        data.loc[:, 'force_13'] = tl.EMA(data['fi'], timeperiod=13)
        # VOL
