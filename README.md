...
- 大幅提升查询性能和数据处理效率
- 支持海量历史数据的高效存储与分析
- 界面新增更新功能，在定时任务的基础上，增加手动更新功能，方便用户随时获取最新数据
![](img/new/index.png)
- 界面支持数据管理，包括数据检查、补齐、下载等功能
![](img/new/download.png)
### 4. 🌐 代理支持优化
...
- **代理配置**：根据购买的代理品牌修改 `proxy_pool.py` 即可，当前示例为青果**按量付费**的短效代理（1~5分钟），如果预期能直接跑起来，可以使用青果代理，地址是https://www.qg.net/?aff=084011 。如果有使用其他代理的，也欢迎补充到readme中来。

- **支持tushare数据源**（开发中）：因为akshare有时候会不稳定，后续会增加tushare数据源的支持，用户可根据需要选择数据源。
### 5. AI agent引入（开发中）
